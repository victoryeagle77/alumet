...
pub struct AmdGpuProbe {
    ...
    /// Metric type base on GPU PCI bus data consumption rate.
    pub pci_usage: TypedMetricId<u64>,
    ...
    fn poll(&mut self, measurement: &mut MeasurementAccumulator, timestamp: Timestamp) -> Result<(), PollError> {
        for device in gather_amd_gpu_device_metrics()? {
            ...
            if let Some(id) = device.metrics.bus_id.clone() {
                ...
                for (data, label) in &device.metrics.engine_usage {
                    if data.supported {
                        measurement.push(
                            MeasurementPoint::new(
                                timestamp,
                            ...
                            .with_attr("engine_type", label.to_string()),
                        );
                    }
                }
                ...
                // GPU PCI bus data consumption metric pushed
                for (data, label) in &device.metrics.pci_usage {
                    if data.supported {
                        measurement.push(
                            MeasurementPoint::new(
                                timestamp,
                            ...
                            .with_attr("pci_data", label.to_string()),
                        );
                    }
                }
