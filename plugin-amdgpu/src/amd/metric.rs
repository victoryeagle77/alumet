...
pub struct AmdGpuMetric {
    ...
    pub engine_usage: Vec<(Features<u64>, String)>,
    /// GPU memory usage (VRAM, GTT) in MB.
    pub memory_usage: Vec<(Features<u64>, String)>,
    /// GPU PCI bus data consumption in B/s.
    pub pci_usage: Vec<(Features<u64>, String)>,
...
impl AmdGpuDevice {
    ...
    /// Update the metrics for this GPU device.
    fn gather_amd_gpu_metrics(&mut self) -> Result<(), PollError> {
        ...
        // Get the GPU usage of hardware engine units by running graphic processes.
        let metric = is_valid(|| amdsmi_get_gpu_activity(device).map_err(AmdError));
        if let Some(data) = metric.value {
            self.metrics.engine_usage = vec![
                (
                    Features::new(Some(data.gfx_activity as u64), true),
                    "activity_graphic_engine".to_string(),
                ),
                (
                    Features::new(Some(data.umc_activity as u64), false),
                    "activity_memory_engine".to_string(),
                ),
                (
                    Features::new(Some(data.mm_activity as u64), false),
                    "activity_average_multimedia_engine".to_string(),
                ),
            ];
        }
        ...
        if let Some((sent, received, _max_pkt_sz)) = metric.value {
            self.metrics.pci_usage = vec![
                (Features::new(Some(sent / 1_000), true), "pci_data_sent".to_string()),
                (
                    Features::new(Some(received / 1_000), true),
                    "pci_data_received".to_string(),
                ),
            ];
        }
