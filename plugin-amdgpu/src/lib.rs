...
impl AlumetPlugin for AmdGpuPlugin {
    ...
    fn start(&mut self, alumet: &mut AlumetPluginStart) -> anyhow::Result<()> {
        ...
        let source = Probe {
            ...
            memory_usage_gtt: alumet.create_metric::<u64>(
                "amd_gpu_memory_usage_gtt",
                PrefixedUnit::mega(Unit::Byte),
                "Get GPU used GTT memory in MB",
            )?,
            memory_usage_vram: alumet.create_metric::<u64>(
                "amd_gpu_memory_usage_vram",
                PrefixedUnit::mega(Unit::Byte),
            ...
            process_counter: alumet.create_metric::<u64>(
                "amd_gpu_counter_compute_process",
                Unit::Unity,
                "Get GPU compute processes number",
            )?,
            process_usage_compute_unit: alumet.create_metric::<u64>(
                "amd_gpu_process_compute_unit_usage",
                Unit::Unity,
                "Get compute unit usage by process in percentage",
            )?,
            process_usage_vram: alumet.create_metric::<u64>(
